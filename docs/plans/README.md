...
| -------------------------------------- | -------------------------- | ---------- | ------------------------------------------------ |
| [01](01-awaken-the-last-third.md)      | Awaken the last third      | —          | `components/motion`, sections 07–09, tokens      |
...
`01` and `02` are independent of everything and of each other. `03` is
independent but must precede `04`, which adds ~36 kB of path data to a chunk
`03` makes lazy, and which `03` spares from having to touch `TrackRegion`.
